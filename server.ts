...
async function startServer() {
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      let validUsername = process.env.AUTH_USERNAME || "www.chevakie.id";
      ...
      const isUsernameMatch = inputUsername === expectedUsername || inputUsername === "ARDI YOKA" || inputUsername === "WWW.CHEVAKIE.ID";
      ...
      if (isUsernameMatch && String(password).trim() === validPassword) {
        ...
        return res.json({
          ...
          token,
          user: {
            username: "www.chevakie.id",
            nama: "www.chevakie.id",
